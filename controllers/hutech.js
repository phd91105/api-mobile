...
let reqpm = require('request-promise');
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        const contentStudent = $(
            ...
            .text()
            .split('-');
        const classStudent = $('#ctl00_ContentPlaceHolder1_ctl00_lblContentLopSV')
            .text()
            .split('-');
        const parentTable = $('#ctl00_ContentPlaceHolder1_ctl00_Table1')
            .children('tbody')
            .children();
        ...
        parentTable.map(function() {
          $(this)
              .children()
              .map(function() {
                const element = $(this).attr('onmouseover');
                if (element) {
                  const item = element.split(',');
                  const startPeriod = parseInt(item[6].slice(0, -1).slice(1));
                  const numberOfPeriods = parseInt(item[7].slice(0, -1).slice(1));
                  schedule.push({
